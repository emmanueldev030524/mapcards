...
function EditableBadge({
  ...
  if (editing) {
    return (
      <input
        ...
        className="w-14 rounded bg-slate-200 px-1.5 py-0.5 text-center text-xs font-bold tabular-nums text-label outline-none focus:ring-1 focus:ring-action"
      />
    )
  }
  ...
  return (
    <button
      ...
      title="Click to edit"
      className="rounded bg-slate-200 px-2 py-0.5 text-xs font-bold tabular-nums text-label transition-colors duration-100 hover:bg-slate-300"
    >
      {value}{suffix}
    </button>
  )
  ...
  return (
    <div className="space-y-2">
      ...
      {hasBoundary && (
        <div className="space-y-1.5">
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium text-label">Fill Opacity</span>
            <EditableBadge
      ...
      {houseCount > 0 && (
        <div className="space-y-1.5">
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium text-label">House Size</span>
            <EditableBadge
          ...
          <div className="mt-1 flex items-center justify-between rounded-lg bg-slate-50 px-3 py-2">
            <p className="text-xs text-muted">Drag houses to reposition</p>
            <button
              onClick={clearAllHouses}
              className="rounded px-2 py-0.5 text-xs font-bold text-red-600 transition-all duration-150 hover:bg-red-100"
            >
              Clear All
            </button>
          </div>
      ...
      {hasBoundary && (
        <div className="mt-3 space-y-2 border-t border-slate-200 pt-3">
          <label className="flex cursor-pointer items-center gap-2.5">
            ...
            <span className="text-sm font-medium text-label">Snap to Grid</span>
          </label>

          {snapToGrid && (
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium text-label">Grid Spacing</span>
                <EditableBadge
